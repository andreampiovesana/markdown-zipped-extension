...
export function activate(context: vscode.ExtensionContext) {
    ...
    const createNewCommand = vscode.commands.registerCommand('mdzEditor.createNew', async (uri: vscode.Uri) => {
    ...
    const openWithCommand = vscode.commands.registerCommand('mdzEditor.openWith', async (uri: vscode.Uri) => {
...
class MdzEditorProvider implements vscode.CustomTextEditorProvider {
    ...
    private async extractMdzContent(uri: vscode.Uri): Promise<MdzContent> {
        try {
    ...
    private async saveMdzFile(uri: vscode.Uri, markdownContent: string, mediaFiles: MediaFile[]): Promise<void> {
        return new Promise((resolve, reject) => {
            const baseName = path.basename(uri.fsPath, '.mdz');
    ...
    private async handleAddMedia(uri: vscode.Uri, webviewPanel: vscode.WebviewPanel): Promise<void> {
    ...
    private getHtmlForWebview(webview: vscode.Webview, content: MdzContent): string {
        return `<!DOCTYPE html>
        <html lang="it">
        <head>
            <meta charset="UTF-8">
...
async function createEmptyMdzFile(filePath: string, baseName: string): Promise<void> {
    return new Promise((resolve, reject) => {
        const zipfile = new yazl.ZipFile();
